...
def load_gen_data2(result_path):
    """加载保存的训练/验证/测试数据列表"""
    # 加载数据列表
...
def gen_data(result_path, seed_num, chns=None):
    ...
    data_train = (data_mi_train, data_hc_train)
    data_val = (data_mi_val, data_hc_val)
    data_test = (data_mi_test, data_hc_test)
    return [data_train, data_val, data_test]


def get_batch(batch_size, train_data, val_data, test_data, window_size=10000, split="train", downsample=True):
    ...
    for sample in mi_batch:
        ...
        for i in range(chn_num):
            segment = sample[i][start : start + window_size]
            # 下采样到500Hz (10000点 -> 5000点)
            if downsample:
                segment = resample(segment, new_window_size)
            # 归一化
            normalized_list.append(minmax_scale(segment))
    ...
    ## 正确  hc为0.1 mi为0.9
    batch_y = [0.9 for _ in range(int(batch_size / 2))]
    ...
    batch_x = np.reshape(batch_x, (-1, chn_num, new_window_size))
...
def gen_data2(result_path, chns=None):

    chns = ["i", "ii", "iii", "avr", "avl", "avf", "v1", "v2", "v3", "v4", "v5", "v6"] if chns == "ALL" else chns
    # path = os.path.join(result_path, 'data')
    hc_train, hc_val, hc_test, mi_train, mi_val, mi_test = load_gen_data2(result_path)
    data_hc_train = []
    data_hc_val = []
    data_hc_test = []
    data_mi_train = []
